...
def main(
    ...
    if 0 < probability < 1:  # Subsampling
        complement = False
        print(f"Subsampling with probability {probability}")
    elif probability > 1:  # Oversampling
        complement = True
        probability = 1 / probability  # Adjust probability for oversampling
        print(f"Oversampling with probability {probability}")
    else:
        raise ValueError("Probability must be between 0 and 1 exclusive, or greater than 1")
